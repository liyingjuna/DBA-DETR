...
from detrex.layers import box_cxcywh_to_xyxy, box_xyxy_to_cxcywh
...
class DBA_DETR(nn.Module):
    ...
    def __init__(
        ...
        # define classification head , box head and iou head
        self.class_embed = nn.Linear(embed_dim, num_classes)
        self.bbox_embed = MLP(embed_dim, embed_dim, 4, 3)
        self.num_classes = num_classes
        self.iou_embed = nn.Linear(embed_dim, 1)
        ...
        self.iou_embed = nn.ModuleList([copy.deepcopy(self.iou_embed) for i in range(num_pred)])
    ...
    def forward(self, batched_inputs):
        ...
        for lvl in range(inter_states.shape[0]):
            ...
            outputs_iou = self.iou_embed[lvl](inter_states[lvl])
        ...
        if self.aux_loss:
            output["aux_outputs"] = self._set_aux_loss(outputs_class, outputs_coord, outputs_iou)

        # prepare two stage output
        interm_coord = enc_reference
        interm_class = self.transformer.decoder.class_embed[-1](enc_state)
        interm_iou = self.transformer.decoder.iou_embed[-1](enc_state)
        ...
        if self.training:
            ...
            if self.vis_period > 0:
                ...
                if storage.iter % self.vis_period == 0:
                    ...
                    if "pred_ious" in output:
                        box_iou = output["pred_ious"]
                        results = self.inference_with_iou(box_cls, box_pred, box_iou, images.image_sizes)
                    else:
    ...
    def dn_post_process(self, outputs_class, outputs_coord, outputs_iou, dn_metas):
        if dn_metas and dn_metas["single_padding"] > 0:
            ...
            if self.aux_loss:
                out["aux_outputs"] = self._set_aux_loss(output_known_class, output_known_coord, output_known_iou)
    ...
    def inference_with_iou(self, box_cls, box_pred, box_iou, image_sizes):
        """
        Arguments:
            box_cls (Tensor): tensor of shape (batch_size, num_queries, K).
                The tensor predicts the classification probability for each query.
        ...
        # box_pred.shape: 1, 300, 4
        prob = box_cls.sigmoid()
        iou = box_iou.sigmoid().repeat(1,1,prob.shape[-1])
        prob = prob * iou
