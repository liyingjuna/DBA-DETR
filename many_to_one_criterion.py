...
from ..losses import  IA_BCE_loss,IOU_IA_BCE_loss,loss_ious
...
class ManyToOneCriterion(BaseCriterion):
    ...
    def get_loss(self, outputs:dict, targets:list, num_boxes:float, layer_spec:int, specify_indices=None):
        ...
        if 'pred_ious' in outputs:
            ...
            src_ious = pred_ious[pos_idx]
            loss_class, loc_weight = IOU_IA_BCE_loss(src_logits, pos_idx_c, src_boxes, src_ious,
        ...
        if 'pred_ious' in outputs:
            pred_ious = outputs['pred_ious']
            src_ious = pred_ious[pos_idx]
            loss_iou = loss_ious(src_ious, src_boxes, target_boxes)
            losses.update({'loss_iou': loss_iou})

        return losses,indices

    # def get_loss_self_dist(self, outputs: dict, outputs_enc: dict, targets: list, num_boxes: float, layer_spec: int, specify_indices=None):
    #     assert 'pred_boxes' in outputs
    #     assert 'pred_logits' in outputs
    #     assert 'pred_boxes' in outputs_enc
    #     # assert (num_boxes  >=0)
    #     assert (layer_spec >= 0)
    #     losses = {}
    #     pred_boxes = outputs['pred_boxes']
    #     src_logits = outputs['pred_logits']
    #     pred_boxes_enc = outputs_enc['pred_boxes']
    #     ##assume 6 layers here
    #     layer_id = layer_spec
    #     # feature = outputs['feature']
    #     device = pred_boxes.device
    ...
    #         pred_ious = outputs['pred_ious']
    #         src_ious = pred_ious[pos_idx]
    #         loss_class, loc_weight = CAA_IA_BCE_loss(src_logits, pos_idx_c, src_boxes, src_ious,
    #                                                  target_boxes, indices, num_boxes,
    #                                                  self.alpha, self.gamma,
    #                                                  w_prime, )
    #     else:
